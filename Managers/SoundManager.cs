...
public class SoundManager
{
    ...
    private readonly Dictionary<(long, long), bool> _soundCache = [];
    ...
    public bool TryPlaySound((long, long) itemId, string label, float volume)
    {
        ...
        {
            _logError($"UniqueLootHelper: Failed to add item {itemId} to sound cache");
            return false;
        }

        _logMessage($"UniqueLootHelper: Attempting to play sound for {label} with ID {itemId}");
    ...
    /// <summary>
    ///    Removes a specific item from the sound cache
    /// </summary>
    public void ClearCacheEntry((long, long) itemId)
    {
        _soundCache.Remove(itemId);
    }
    ...
    public void CleanupCache(IEnumerable<(long, long)> currentItemIds)
    {
        ...
        List<(long, long)> keysToRemove = new(_soundCache.Count / 10); // Approximate estimate for capacity

        foreach ((long, long) id in _soundCache.Keys)
        {
        ...
        foreach ((long, long) key in keysToRemove)
        {
