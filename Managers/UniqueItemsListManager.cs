...
public class UniqueItemsListManager
{
    ...
    public bool RegenerateUniqueItemsList()
    {
        ...
        try
        {
            ...
            Task.Run(async () =>
            {
                try
                {
                    ...
                    if (completed)
                    {
                        _logMessage($"UniqueLootHelper: Regeneration process completed with exit code {process.ExitCode}");
                    ...
                    {
                        _logError("UniqueLootHelper: Regeneration process timed out after 60 seconds");
                        try
                        {
                            process.Kill();
                        }
                ...
                {
                    _logError($"UniqueLootHelper: Error waiting for regeneration process: {ex.Message}");
                }
                finally
                {
                    IsRegenerating = false;
        ...
        {
            _logError($"UniqueLootHelper: Failed to regenerate unique items list: {ex.Message}");
            IsRegenerating = false;
            return false;
        }
    }
